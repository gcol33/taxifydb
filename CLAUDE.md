...
(`gcol33/oft`). It is not held in this checkout's `datasets/`; it sits in a
separate `datasets/` workspace tree outside this checkout, and the single oft-side
...
| colxr | flat DwC-A TSV (ChecklistBank) | COL Extended Release, the taxonomy GBIF.org serves by default; canonical `scientificName` with authorship in its own column, classification denormalized on every row, alphanumeric IDs; monthly, so the release is resolved from the ChecklistBank API not a fixed URL Carries a `gbif_key` column, the legacy GBIF taxon key(s) of the same name, crosswalked from the `gbif` backbone (#59; see the note below) |
...
**COL XR `gbif_key` (#59).** GBIF serves occurrence records only for the numeric keys of its legacy backbone, and COL XR's alphanumeric IDs return 0 records; neither the DwC-A nor ChecklistBank's usage records carry a mapping between them. `build_colxr()` therefore adds a `gbif_key` column beside the untouched COL XR `taxon_id`, derived from the `gbif` backbone by `colxr_gbif_crosswalk()` (explicit `gbif_path`, else the published build in the manifest, since the keys must describe the backbone taxify serves, else a local `output/gbif/gbif.vtr` when the manifest lists none). Names are matched on canonical name, rank and authorship, with authorship compared without years, brackets, case or accents; a COL XR usage whose full authorship misses GBIF's tries its authors outside the brackets, then the name and rank alone where GBIF records no author. A GBIF usage naming a different author is never matched, and a GBIF synonym contributes its accepted taxon's key, since GBIF files a synonym's records under the accepted taxon. Several GBIF usages sharing a name give a `|`-delimited, ascending set of keys. Measured against `output/gbif` (2026-08-28) and `output/colxr` (2026-08-26): 89.2% of accepted species carry a key (exact authorship alone reached 15.0%); the rest are names newer than GBIF's backbone, and viruses, which GBIF does not hold. The md5 of the `gbif` build it read is recorded in the `.meta` as `gbif_key_source`.
...
**GBIF infraspecific marker note (#45).** GBIF's `canonical_name` renders an infraspecific taxon without its rank connecting term (`Erica tenella tenella` for `Erica tenella var. tenella`, `Anacamptis morio litardierei` for `... nothosubsp. litardierei`), a rendering no other backbone and no botanical source produces, so anything keyed on the name misses — 108 of the 109 rank-marker residual pairs measured against a rebuilt wcvp were GBIF. It is not enrichment-specific: `accepted_name` carried the same dropped marker into every `add_data()`/`reconcile()` join. `gbif_render_infraspecific()` reads the marker back from the full `scientific_name` (which carries it) rather than mapping it from `rank`, because `rank` cannot tell a botanical subspecies from a zoological one and a zoological trinomial (`Panthera leo persica`) is correctly marker-less — so the marker is inserted only where the name's own rendering uses one. The marker is anchored on the epithet (the connecting term written immediately before the infraspecific epithet), which keeps a `f.` forma marker apart from an `L. f.` filius author. The reconstruction runs in `normalize_gbif` before `precompute_keys`, so `key_ci`/`key_normalized`, the embedded `accepted_name` and the genus register all pick up the corrected canonical. `.infraspecific_markers` (in `R/normalize.R`) is the shared marker vocabulary, used here and by Euro+Med's epithet/authorship parsing. **Requires a GBIF backbone rebuild + republish (build-heavy is dormant, so by hand) to reach published assets.**
