...
#' species.
#' @noRd
.pivot_species_traits <- function(long, spec) {
...
parse_globtherm <- function(path) {
  ...
  out <- data.frame(
    canonical_name      = trimws(paste(chr(1L), chr(2L))),
...
parse_gwdd <- function(path) {
  ...
  out <- data.frame(
    canonical_name         = trimws(as.character(df$species)),
