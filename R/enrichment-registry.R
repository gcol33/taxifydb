...
.enrichment_build_registry <- list(
  ...
  kew_cvalues = list(
    ...
    source_doi  = NA_character_,
    # The database's own pages state no licence, only the Kew copyright notice
    # and a citation request, so this label rests on two things rather than on a
    # licence badge. Kew's Terms and Conditions reserve rights generally but
    # carve out that "our pages containing science data and digital resources
    # are governed by separate terms of use. Data and information on these pages
    # are available under the Creative Commons Attribution Licence (CC-BY)", and
    # cvalues.science.kew.org is such a page. Those separate terms then add:
    # "Where we present scientific data generated by others, we impose no
    # additional restriction on the use of the contributed data than those
    # provided by the data owner." The contents here are measurements published
    # by others, which the contributing authors do not restrict, so neither does
    # Kew. Every value carries its original_reference so the measurement a user
    # relies on is attributed to the paper that made it.
    license     = "CC BY",
    attribution = paste0(
