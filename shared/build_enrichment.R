...
#'
#' Sorts by canonical_name, writes .vtr, creates hash indexes on
...
build_enrichment_vtr <- function(df, vtr_path, name, version, source_url,
  ...
  # Sort by canonical_name for index locality

  df <- df[order(df$canonical_name, na.last = TRUE), ]
  rownames(df) <- NULL

  # Drop rows with NA canonical_name
  ...
  # Extract available groups for group-based enrichments
